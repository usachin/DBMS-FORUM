        ...
        echo 'No posts defined yet.';
         ...
         while($row = mysqli_fetch_assoc($result))
            {  $row1 = mysqli_fetch_assoc($result1);            
                ...
                echo $row['Content'];
                $pid = $row['post_Id'];
                 echo "</br><a href = 'delete1.php?id={$pid}'> <button class = 'item' type='button' name='delete' >DELETE</button></a>";
                echo '</td>';
                echo '</tr>';
            }
        }
...
echo "<a class='item' id = 'post' href='orderbyvote.php?id={$id}'>OrderPostByVote</a></div>";
